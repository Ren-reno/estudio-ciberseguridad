...
- **1.1** — *Objetivo:* explicar en tus palabras por qué el three-way handshake sincroniza números de secuencia y no es solo un saludo de conexión. *Prerrequisitos:* ninguno formal (primera sesión del módulo). El Chequeo de Bases no cubrió Módulo 1 a fondo y 1.rev confirmó el hueco: sobre predicción de ISN ya había una intuición general correcta ("suplantación"), pero sin el mecanismo de sincronización que la sostiene ni la distinción off-path/on-path (esa es de 1.2, no de acá) — arrancar aprovechando esa intuición, no desde cero absoluto. *Conceptos nuevos:* three-way handshake, número de secuencia (ISN), sincronización de estado en TCP.
- **1.2** — *Objetivo:* explicar qué es la predicción de ISN y qué hace cada flag (SYN/ACK/FIN/RST/PSH/URG) de un segmento TCP. *Prerrequisitos:* handshake y número de secuencia (1.1) — quedó sólido: el cálculo de ack en las 2 direcciones fue correcto, con una imprecisión puntual de nombre (etiquetó el ack como "seq"); repasar esa distinción de nombre al arranque, no como clase aparte. La verificación (1.rev) dio 0/3 en session hijacking — no dar por sabida la distinción on-path/off-path, definirla explícitamente. *Conceptos nuevos:* predicción de ISN, hijacking on-path vs off-path, flags TCP.
